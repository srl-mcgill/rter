...
.controller('TermViewCtrl', function($scope, $filter, $timeout, Alerter, ItemCache, UpdateItemDialog, CloseupItemDialog, TermViewRemote, TaxonomyRankingCache) {
	...
	$scope.getLeftValue = function(item) {
		if(item.DelayedHeading < -90)
			item.DelayedHeading = -90;
		if(item.DelayedHeading > 90)
			item.DelayedHeading = 90;
		return (item.DelayedHeading + 90) * (1000 - 40 - $scope.panoramaItemWidth) / 180 + 20;
	}
	...
	$scope.mapCenter = new google.maps.LatLng(27.94202, -82.45514);
